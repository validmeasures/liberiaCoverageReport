...
Data analysis was performed using R language for statistical computing [@R:2019rr].
...
Spatial distribution of MNP supplementation coverage was across the board low in Greater Monrovia at baseline. By endline, areas in the north and central areas of Monrovia have improved coverage (see Figure \@ref(@mnp1map)). Spatial distribution of MNP supplementation coverage was across the board extremely low in Grand Bassa at baseline. By endline, areas in the north and central areas of Grand Bassa have improved coverage.
...
## Concatenate
cmamDF <- data.frame(rbind(x, y))

cmamDF$county <- factor(cmamDF$county, levels = c("Greater Monrovia", "Grand Bassa"))
cmamDF$varNames <- factor(cmamDF$varNames, levels = unique(varNames))

ggplot(data = cmamDF, mapping = aes(x = round, y = estimate * 100)) +
  geom_col(color = "#85010C", fill = "#85010C", width = 0.7, alpha = 0.7) +
  facet_grid(county ~ varNames) +
  labs(x = "", y = "%") +
  scale_y_continuous(limits = c(0, 100),
...
x[ , c("estimate", "lcl", "ucl")] <- round(y[ , c("estimate", "lcl", "ucl")] * 100, digits = 2)
...
ggplot(data = y, aes(x = indicator, y = count)) +
  ...
  scale_x_discrete(labels = c("Not aware of\ntreatment programme", 
                              "Too far",
                              "Does not recognise that child is\nsick/undernourished",
